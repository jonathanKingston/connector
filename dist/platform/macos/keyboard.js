...
export async function keyboardType(options) {
    const { text } = options;
...
export async function keyboardKey(options) {
    const { key, modifiers } = options;
    const keyLower = key.toLowerCase();
    const keyCode = KEY_CODES[keyLower];
    if (keyCode === undefined) {
        throw new Error(`Unknown key: "${key}". Supported keys: ${Object.keys(KEY_CODES).join(", ")}`);
    }
...
`;
        ...
        const modifierList = modifiers
            ...
            if (!mapped) {
                throw new Error(`Unknown modifier: "${m}". Supported: ${Object.keys(MODIFIER_MAP).join(", ")}`);
            }
            return `"${mapped}"`;
        })
            .join(", ");
